// Seed a curated starter brand catalog into whatever DB_PATH points at (run with --env-file=.env to
// hit the real app DB). Idempotent: upsertBrands merges on UNIQUE(name) (ON CONFLICT DO UPDATE), so
// re-running refreshes rather than duplicates. The `categories.main` names BECOME the sector vocabulary
// (categoryFacets) that Creator IQ scores against and Brand Match ranks on — keep them clean + canonical.
// The HikerAPI snowball can grow/enrich this later (it fires automatically when a creator's sectors are
// thin); this just gives the catalog a real floor so the pipeline runs in the app today.
//   node --env-file=.env scripts/seed-brands.ts
const { db } = await import('../src/db.ts')
const { upsertBrands, categoryFacets, brandCount } = await import('../src/brands.ts')
...
const t = db.prepare('SELECT id FROM tenants ORDER BY created_at LIMIT 1').get() as { id: string } | undefined
if (!t) throw new Error('no tenant in this DB to attribute brand provenance to (sign up first)')

const res = upsertBrands(
  t.id,
  BRANDS.map(([name, instagramHandle, followers, website, locationCountry, main]) => ({
    name, instagramHandle, followers, website, locationCountry,
    categories: { main, secondary: [] }, source: 'manual' as const,
  })),
)

console.log(`brand catalog: +${res.inserted} new, ${res.updated} updated → ${brandCount()} total`)
